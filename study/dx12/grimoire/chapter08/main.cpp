...
#endif
{
	...
	std::fread(&material_num, sizeof(material_num), 1, fp);
	std::vector<PMDMaterial> pmd_materials(material_num);
	std::fread(pmd_materials.data(), pmd_materials.size() * sizeof(PMDMaterial), 1, fp);
	// マテリアルコピー
	std::vector<Material> materials(pmd_materials.size());
	for (int i = 0; i < pmd_materials.size(); ++i)
	{
		materials[i].indices_num          = pmd_materials[i].indices_num;
	...
	// マテリアルバッファ
	auto material_buffer_size = sizeof(MaterialForHlsl);
	material_buffer_size = (material_buffer_size + 0xff) & ~0xff;

	ID3D12Resource* material_buffer = nullptr;
	if (FAILED(p_device->CreateCommittedResource(&CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_UPLOAD), D3D12_HEAP_FLAG_NONE, &CD3DX12_RESOURCE_DESC::Buffer(material_buffer_size * material_num), D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&material_buffer))))
	{
		OUT();
	}
	...
	if (FAILED(material_buffer->Map(0, nullptr, (void**)&map_material))) { OUT(); }
	for (auto& m : materials)
	{
		*((MaterialForHlsl*)map_material) = m.material;
		map_material += material_buffer_size;
	}
	material_buffer->Unmap(0, nullptr);

	// マテリアルディスクリプタヒープ
	ID3D12DescriptorHeap* material_desc_heap = nullptr;
	D3D12_DESCRIPTOR_HEAP_DESC mat_desc_heap_desc = {};
	mat_desc_heap_desc.Flags = D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE;
	mat_desc_heap_desc.NodeMask = 0;
	mat_desc_heap_desc.NumDescriptors = material_num;

	mat_desc_heap_desc.Type = D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV;
	if (FAILED(p_device->CreateDescriptorHeap(&mat_desc_heap_desc, IID_PPV_ARGS(&material_desc_heap)))) { OUT(); }

	// マテリアルビュー
	D3D12_CONSTANT_BUFFER_VIEW_DESC mat_cbv_desc = {};
	mat_cbv_desc.BufferLocation = material_buffer->GetGPUVirtualAddress();
	mat_cbv_desc.SizeInBytes = material_buffer_size;
	
	auto mat_desc_heap_h = material_desc_heap->GetCPUDescriptorHandleForHeapStart();

	for (int i = 0; i < material_num; ++i)
	{
		p_device->CreateConstantBufferView(&mat_cbv_desc, mat_desc_heap_h);
		mat_desc_heap_h.ptr += p_device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
		mat_cbv_desc.BufferLocation += material_buffer_size;
	}
	...
	desc_tbl_range[0].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_CBV;
	...
	desc_tbl_range[1].BaseShaderRegister = 1;
	...
	root_param[0].DescriptorTable.pDescriptorRanges = &desc_tbl_range[0];
	...
	while (true)
	{
		...
		p_cmd_list->SetDescriptorHeaps(1, &material_desc_heap);
		p_cmd_list->SetGraphicsRootDescriptorTable(1, material_desc_heap->GetGPUDescriptorHandleForHeapStart());
